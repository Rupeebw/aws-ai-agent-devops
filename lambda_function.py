...
def ec2_start(intent_request):
    ...
    if response['ResponseMetadata']['HTTPStatusCode'] == 200 and response['StartingInstances'][0]['CurrentState']['Name'] == "pending":
        return close(intent_request['sessionAttributes'],
                'Fulfilled',
                {'contentType':'PlainText',
                'content': 'Your Instance {} is successfully started'.format(instance_id)})
    else:
        return close(intent_request['sessionAttributes'],
                'Fulfilled',
                {'contentType':'PlainText',
...
def ec2_stop(intent_request):
    ...
    if response['ResponseMetadata']['HTTPStatusCode'] == 200 and response['StoppingInstances'][0]['CurrentState']['Name'] == "stopping":
        return close(intent_request['sessionAttributes'],
                'Fulfilled',
                {'contentType':'PlainText',
                'content': 'Your Instance {} is successfully stopped'.format(instance_id)})
    else:
        return close(intent_request['sessionAttributes'],
                'Fulfilled',
                {'contentType':'PlainText',
...
def s3_list(intent_request):
    ...
    return close(intent_request['sessionAttributes'],
                'Fulfilled',
                {'contentType':'PlainText',
                'content': 'Please find the list of S3 Buckets :  {}'.format(str(buckets))})
...
def ec2_setup(intent_request):
    ...
    instance_type = get_slots(intent_request)["Instanc"]
    ...
    return close(intent_request['sessionAttributes'],
                'Fulfilled',
                {'contentType':'PlainText',
                'content': 'Your Instance is created and instance id is : {}'.format(instances[0].id)})
